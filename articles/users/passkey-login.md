...
Passkey login, formerly known as Web Authentication or WebAuthn for 
short, allows a user to securely log into a site without using a password 
though a username is still needed. It uses strong cryptography in a manner 
which is extremely resistant to the most common problems of passwords:
...
Passkey login is not just very secure; it is also very user friendly! You no
...
Passkey login only works over HTTPS and only when your site is using a valid,
...
Passkey login uses public key cryptography, the same proven technology that
...
Passkey login is the future of authentication. Easy, secure and hassle-free.
...
![photograph of Hardware device](../../../en/images/users/passkey-login/01-hardware-device.jpg)

Passkey login uses a system plugin that is enabled by default. A **Sign in
with passkey** button will be present in default Joomla 4 and later login
screens, as illustrated in the Administrator login screen:

![secure administrator login form](../../../en/images/users/passkey-login/02-login-form.png)
...
- Select **User Menu → Edit Account → Passkey Login** to see the form, 
  initially with no authenticators registered.
...
![secure administrator login hardware prompt](../../../en/images/users/passkey-login/03-hardware-prompt.png)
...
![secure administrator login registered authenticator](../../../en/images/users/passkey-login/04-registered-authenticator.png)
...
### Passkey login compatible browsers
...
browsers still lack support for Passkey login.
...
Username and Passkey login.

## How To Disable the Plugin

If you do not wish to allow Passkey login just go to the list of plugins and
find **System - Passkey (Passwordless) Login** plugin in the System group and
disable it. There are no parameters to set.

## Server Requirements

For Passkey login to work the following preconditions must be met:
...
### I can't see the *Sign in with passkey* button

You are not accessing your site under HTTPS. Passkey login is only available
for HTTPS sites with a valid certificate. This is a security precaution
baked into the Passkey login standard. The plugin actually checks if the site
...
### I still need to provide a username. Isn't Passkey login supposed to get rid of usernames?

Not really, no. The current specification of Passkey login does not provide
identity management. Web browsers require us to send them a list of
acceptable Passkey login public keys during the login phase. This means that
we need your username to fetch them.

That said, using Passkey login finally makes it clear that usernames *are not
...
It is a bug but not with the Passkey login plugin itself.
...
Safari only included support for Passkey login as an experimental feature and
not quite finished.
...
trick. This is a security precaution in the Passkey login standard itself:
...
The linchpin of Passkey login is the absolute secrecy of the private key. It
...
Passkey login) is under €20 on Amazon you can use a hardware authenticator in
...
the formal name of registering an authenticator per the Passkey login
...
Passkey login is an entirely different authentication solution which has none
...
Passkey login credential is tied to the exact domain name it was issued for
(yes, if you use multiple domains for your site or transfer your site to
another domain you'll need to re-register all your Passkey login
authenticators – you got that right!). As a result, authentication with
Passkey login is incredibly secure and supersedes the reasons that
necessitated TFA. This means that if you successfully authenticate using
Passkey login the TFA secret key needs not be –and is therefore not– checked
at all.

In an ideal world, you would only be able to log into your site using
Passkey login. This is a feature we are working on and you may not want to
enable; after all, if your domain name changes or you lose access to or
reset all of your Passkey login authenticators you'd be locked out of your
...
### Isn't TFA good enough? Why do we need Passkey login?
...
Passkey login greatly improves the user experience. Major browsers embraced
Passkey login and offer a compelling user experience, guiding the users into
using authenticators successfully. Logging in with Passkey login is more
...
Where Passkey login is truly killing it is at the security front. By virtue
...
Passkey login with FIDO2 authenticators is more theft- and loss-resistant
...
Passkey login system plugin implements this functionality. Such buttons can
